...
select apellido1,apellido2,nombre,fecha_nacimiento from persona
 ...
 inner join departamento on profesor.id_departamento = departamento.id order by apellido1,apellido2,persona.nombre;
 ...
 select distinct apellido1,apellido2,nombre, curso_escolar.anyo_inicio,curso_escolar.anyo_fin from persona 
 ...
 order by departamento.nombre,apellido1,apellido2,nombre;
 
 select apellido1,apellido2,persona.nombre,departamento.nombre from persona
 left join profesor on persona.id = profesor.id_profesor
 left join departamento on profesor.id_departamento = departamento.id
...
select apellido1,apellido2,persona.nombre,asignatura.nombre from persona
left join profesor on persona.id = profesor.id_profesor
...
select departamento.nombre,curso_escolar.anyo_inicio from departamento
left join profesor on departamento.id = profesor.id_departamento
left join asignatura on profesor.id_profesor = asignatura.id_profesor
...
select grado.nombre,count(asignatura.id) as asignaturas from grado
...
select grado.nombre,count(asignatura.id) as asignaturas from grado
left join asignatura on asignatura.id_grado = grado.id
group by grado.nombre having asignaturas > 40;

select grado.nombre,asignatura.tipo,count(asignatura.creditos) as creditos from grado
inner join asignatura on asignatura.id_grado = grado.id
group by asignatura.tipo,grado.nombre
order by creditos desc;

select anyo_inicio,count(persona.id) as alumnos from curso_escolar
...
select persona.id,persona.nombre,apellido1,apellido2,count(asignatura.id) as asignaturas from persona
left join profesor on persona.id = profesor.id_profesor
left join asignatura on profesor.id_profesor = asignatura.id_profesor
group by persona.id
order by asignaturas desc;
...
select * from persona
where tipo ='alumno' order by fecha_nacimiento limit 1;

select apellido2,apellido1,persona.nombre,asignatura.id from persona
 left join profesor on persona.id = profesor.id_profesor
left join asignatura on profesor.id_profesor = asignatura.id_profesor
